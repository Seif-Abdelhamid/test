...
const bodyContent = `		<!-- Site Notifications Container -->
<!-- Main Site Header -->
...
<!-- Hero Section: Full-Height Video Background -->
				...
				<div class="gallery gallery--fit">
					<div class="gallery__item gallery__item--dimmed">
						<video autoplay muted loop playsinline>
							<source src="assets/images/OMGyro.mp4" type="video/mp4">
						</video>
			...
			<section class="modern-content-section revealable">
				<div class="modern-content-wrapper">
					<div class="modern-content-image">
						<div class="image-container" style="background-image: url('assets/images/Platters/Compo-Platter.jpeg');"></div>
					</div>
					<div class="modern-content-text">
						...
						<a href="locations/index.html" class="modern-btn" target="_parent">Find a Location</a>
					</div>
				</div>
			</section>
			...
			<section class="modern-content-section modern-content-reverse revealable">
				<div class="modern-content-wrapper">
					<div class="modern-content-image">
						<div class="image-container" style="background-image: url('assets/images/The-store.jpg');"></div>
					</div>
					<div class="modern-content-text">
			...
			<section class="modern-content-section revealable">
				<div class="modern-content-wrapper">
					<div class="modern-content-image">
						<div class="image-container" style="background-image: url('assets/images/Sauce-Bible.jpeg');"></div>
					</div>
					<div class="modern-content-text">
						...
						<a href="store/index.html" class="modern-btn" target="_parent">Explore Our Sauce Bible</a>
					</div>
				</div>
			</section>
...
<!-- Modern Scroll Animations -->
		<script>
			document.addEventListener('DOMContentLoaded', function() {
				...
				const observer = new IntersectionObserver(function(entries) {
					entries.forEach(entry => {
						if (entry.isIntersecting) {
							...
							const cols = entry.target.querySelectorAll('.c-split__col');
				...
				// Observe all revealable sections
				const revealables = document.querySelectorAll('.revealable');
				revealables.forEach(el => {
					observer.observe(el);
				});

				// Feature Cards Animation (Old)
				...
				// Modern Feature Cards Animation
				const modernFeatureCards = document.querySelectorAll('.feature-modern-card');
				const modernFeatureObserver = new IntersectionObserver(function(entries) {
					entries.forEach((entry, index) => {
						if (entry.isIntersecting) {
							setTimeout(() => {
								entry.target.classList.add('revealed');
							}, index * 150);
							modernFeatureObserver.unobserve(entry.target);
						}
					});
				}, { threshold: 0.1, rootMargin: '0px 0px -50px 0px' });

				modernFeatureCards.forEach(card => {
					modernFeatureObserver.observe(card);
				});

				// Hero video is handled by CSS

				// Smooth reveal on page load for hero
				setTimeout(function() {
					const heroContent = document.querySelector('.gallery__content');
					if (heroContent) {
				...
				const buttons = document.querySelectorAll('.btn-brand-alt, .btn-brand');
				buttons.forEach(btn => {
				...
				// FORCE remove all section separation lines/borders
				function removeSectionLines() {
					const allSections = document.querySelectorAll('.modern-content-section');
				...
				removeSectionLines();
				// Also run after a delay to catch any dynamically applied styles
				setTimeout(removeSectionLines, 500);
				setTimeout(removeSectionLines, 1500);
				setTimeout(removeSectionLines, 3000);
...
export default function HomePage() {
  useEffect(() => {
    ...
    // Inject JS
    const jsFiles = ['/assets/js/mobile-panel.js', '/assets/js/lenis-smooth.js', '/assets/js/sauce-bottle-fill.js'];
    jsFiles.forEach((src) => {
